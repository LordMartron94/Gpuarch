...
func vulkanBindingSpecIRPrint(ir VulkanSpecIR) {
	fmt.Printf("Vulkan spec IR: %d basetypes, %d API constants, %d enum types, %d flag types, %d funcpointers, %d commands, %d handles, %d structs/unions, %d api versions, %d extensions, %d features\n",
		len(ir.Basetypes), len(ir.Constants.Values), len(ir.Enums), len(ir.Flags), len(ir.Funcpointers), len(ir.Commands), len(ir.Handles), len(ir.Structs),
